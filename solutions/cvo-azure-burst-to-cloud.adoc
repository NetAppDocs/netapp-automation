...
* You must download the link:https://console.bluexp.netapp.com/automationCatalog[CVO Azure - Burst to cloud^] automation solution through the BlueXP web UI. The solution is packaged as `CVO-Azure-Burst-To-Cloud.zip`.
...
. After you extract the contents, navigate to the `CVO_Azure_Deployment` folder and  run the following command to view the files:
+
[source,cli]
 ls -la
+
