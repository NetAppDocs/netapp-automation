...
. Locate the `cvo_azure_flexcache_ubuntu_image_latest.tar.gz` file. This contains the Docker image required to deploy Cloud Volumes ONTAP on Azure.
+
Untar the file:
+
[source,cli]
...
== Step 6: Deploy Cloud Volumes ONTAP on Azure
