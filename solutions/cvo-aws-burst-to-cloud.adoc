...
[.lead]
This article supports the NetApp Cloud Volumes ONTAP for AWS Automation Solution, which is available to NetApp customers from the NetApp Console automation hub.
...
* You must download the link:https://console.netapp.com/automationHub[Cloud Volumes ONTAP AWS - Burst to cloud^] automation solution through the Console web UI. The solution is packaged as `cvo_aws_flexcache.zip`.
