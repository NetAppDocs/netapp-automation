...
* You must download the link:https://console.bluexp.netapp.com/automationCatalog[CVO AWS - Burst to cloud^] automation solution through the BlueXP web UI. The solution is packaged as `cvo_aws_flexcache.zip`.
...
. After you extract the contents, navigate to the `CVO_Aws_Deployment` folder and run the following command to view the files:
+
[source,cli]
 ls -la
+
