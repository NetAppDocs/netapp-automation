...
You must create a local variable file for authentication using the access and secret key. The add the file to the `.env` file.
...
. Add the absolute file paths to the `.env` file using the `AWS_CREDS` variable. For example:
+
...
+
Make sure to update the volume name (last parameter) before running the command.
+
[source,cli]
...
+
You can also add an NFS share as the external volume using a command such as:
+
...
+
See https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/fsx_ontap_file_system for more information.
...
+
This command creates two containers. The first container delete flexcache and the second container delete's FSxN
