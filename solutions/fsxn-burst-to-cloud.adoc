...
. Install the Docker software and verify the installation.
+
[source,cli]
...
== Step 4: Create environment variable file for AWS credentials

You must create a local variable file for authentication using the access and secret key. This file is added to the `.env` file.
...
. Navigate to the `XXX` folder and update the access and secret key in the credentials file.
...
+
`PERSISTENT_VOL=path/to/external/volume:/aws_fsx_flexcache`.
+
Remember to keep the existing file contents and colon formatting. For example:
+
[source,cli]
PERSISTENT_VOL=aws_fsxn_volume:/aws_fsx_flexchace
...
+
This creates two containers. The first container deploys FSxN and the second container creates the cluster peering, SVM peering, destination volume, and FlexCache.
...
+
This command gives you the output in real-time, but has been configured to capture the logs through the file deployment.log. You can change the name of these log files by editing `.env` file and updating the variables `DEPLOYMENT_LOGS`.
...
+
This creates two containers. The first container delete flexcache and the second container delete's FSxN
