...
* `vault.yml` - You must decrypt the file using a temporary password in order to modify the file.
+
...
+
.. When prompted for the vault password, enter the temporary password "NetApp123!"
+
.. Modify the ONTAP cluster, username and password values in the file to suit your environment. 
