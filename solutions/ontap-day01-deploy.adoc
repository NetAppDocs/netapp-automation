...
+
Open the `tutorial-requests.yml` file using an editor and copy the contents from lines 11 to line 165. Paste the contents under the `raw service request` variable in the new `cluster_initial.yml` file, as shown in the following examples:
+
...
.. Manually define the `raw_service_request` variable.
+
Open the `tutorial-requests.yml` file using an editor and copy the contents from lines 179 to line 222. Paste the contents under the `raw service request` variable in the new `svm_initial.yml` file, as shown in the following examples:
+
...
The following example commands dynamically defines a service request for both clusters, using the `tutorial-requests.yml` file as an example:
...
NOTE: At this stage, you should have already decrypted and modified the `vault.yml` file and it should be encrypted with your new password. 
