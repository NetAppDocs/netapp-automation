...
The service definition and the request work together to determine the action. If you provide a service request for a microservice that is not in the service definition it is skipped. If you pass a service request with one or more microservices defined in the service definition, but omitted from the request, it is skipped.

The execution.yml playbook evaluates the service definition by running through the list of microservices in the order listed. If there is an entry in the request with a dictionary key matching the “args” entry contained in the microservice definition of the service definition, then it is executed. If there is no matching entry in the service request, it is skipped with no error.
...
== Step 5: Optionally, provision NAS
...
[IMPORTANT]
====
* If the `logic_operation` variable is not defined, the `logic.yml` file does not import any file from logic-tasks. This means the `raw_service_request` must be defined outside of Ansible and passed to the framework upon execution. 
...
+
----
ansible-playbook -i inventory/hosts -e cluster2provCluster
...
The following are some example commands to perform a quick run of certain operations:

.<Need name of operations>
...
Temporary Password to decrypt the vault.yml file and it's corresponding vault password- NetApp123!
