...
+
.. Navigate to the `tutorial-requests.yml` file and copy the contents from lines 11 to line 165. Paste the contents under the `raw service request` variable in the new `cluster_initial.yml` file, as shown in the following example:
+
.Show example
[%collapsible]
====
