...
+
You can use one of the following options to define the raw_service_request` for `cluster_initial` in the `logic-tasks` folder:
...
----
+
.. Navigate to the `tutorial-requests.txt` file and copy the contents from lines 11 to line 165. Paste the contents under the `raw service request` variable in the new `cluster_initial.yml` file, as shown in the following example:
+
.Show example
[%collapsible]
====
...
NOTE: "NetApp123!" is a temporary password to decrypt the `vault.yml` file and the corresponding vault password. After first use, you are required to change this to your own password. 
