...
* Manually define the `raw_service_request` variable.
+
.. Navigate to the `tutorial-requests.yml` file and copy the contents from lines 179 to line 222. Paste the contents under the `raw service request` variable in the new `svm_initial.yml` file, as shown in the following examples:
+
...
* A tasks file name in the `logic-tasks` folder must match the value of the `logic_operation` variable without the .yml extension.
