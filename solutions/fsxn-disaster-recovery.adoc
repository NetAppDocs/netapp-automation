...
* You need to download the https://console.bluexp.netapp.com/automationCatalog[FSxN - Disaster recovery^] automation solution through the BlueXP web user interface. The solution is packaged as `FSxN_DR.zip`. This zip contains the `AWS_FSxN_Bck_Prov.zip` file that you will use to deploy the solution described in this document.
