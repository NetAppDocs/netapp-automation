...
* You need to download the https://console.netapp.com/automationCatalog[Amazon FSx for NetApp ONTAP - Disaster recovery^] automation solution through the NetApp Console web UI. The solution is packaged as `FSxN_DR.zip`. This zip contains the `AWS_FSxN_Bck_Prov.zip` file that you will use to deploy the solution described in this document.
